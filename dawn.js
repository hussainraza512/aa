...
// Add event listener to body for click outside of flood1
body.addEventListener("click", (event) => {
  // Check if the clicked element is not within flood1 or bar
  if (!flood1.contains(event.target) && !bar.contains(event.target)) {
    flood1.style.display = "none";
  }
});
...
body.addEventListener("click", (event) => {
  // Check if the clicked element is not within flood1 or bar
  if (!flood.contains(event.target) && !bar.contains(event.target)) {
    flood.style.display = "none";
  }
});
